...
export function createBikeDetail({
  ...
  // Lightbox-instans (oprettes nedenfor — afhænger af galleryGoto)
  let _lightbox = null;
  ...
  /* ── Lightbox: extraheret til js/bike-detail-lightbox.js ── */
  _lightbox = createBikeDetailLightbox({ galleryGoto });
  ...
  /* ============================================================
     ...
     Skal kaldes efter DOM er klar — wirer keyboard-handlers og
     binder lightbox-instansens initLightboxGestures (extraheret i
     bike-detail-lightbox.js) til DOMContentLoaded.
     ============================================================ */

  function setupLightboxEvents() {
    document.addEventListener('DOMContentLoaded', _lightbox.initLightboxGestures);
    if (document.readyState !== 'loading') _lightbox.initLightboxGestures();

    // Escape lukker lightbox først (før andre modaler)
    document.addEventListener('keydown', (e) => {
      const lb = document.getElementById('lightbox-modal');
      if (!lb || !lb.classList.contains('open')) return;
      if (e.key === 'Escape') { e.stopPropagation(); _lightbox.closeLightbox(); }
      else if (e.key === 'ArrowLeft') _lightbox.lightboxNav(-1);
      else if (e.key === 'ArrowRight') _lightbox.lightboxNav(1);
    }, true);
  ...
  function registerWindowExports() {
    ...
    window.openLightbox               = _lightbox.openLightbox;
    window.closeLightbox              = _lightbox.closeLightbox;
    window.lightboxNav                = _lightbox.lightboxNav;
    window.lightboxResetZoom          = _lightbox.lightboxResetZoom;
